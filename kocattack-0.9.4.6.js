...
var KOCAttack={
	...
	*/	
	...
	IsFirstAttackAtLocation:function(x,y) {
		...
		for (var march in current_marches) {
			if(current_marches[march]['toXCoord'] == x && current_marches[march]['toYCoord'] == y){
				...
				if(elapsedTime > 30){
					this.Log("Suicide wave attack was sent more than 30 seconds ago. Suicide wave should be re-sent! This is the first attack sent to ("+x+","+y+")!");
					return true;
				}
	...
	SetAttackFromGuiXY:function(x,y,box,isSuicideWave,locationType,resetTime) {
		...
		if(!locationType){
			var locationType = "";
			// Look at the radio options to determine which attack location type we're dealing with
	...
	*/
	...
	FindBiggestCampAttack:function(attacks,currentTroops,currentResources) {
		...
		for(var a=0; a<attacks.length; a++) {
			...
			if(levelInfo.type == ""){levelInfo.type = "Camp";} // Default to camp
			...
			if(levelInfo.type=='City' || levelInfo.type=='Transport') {
				if(attack.a.type==0) {
			...
			} else if(levelInfo.type=='' || levelInfo.type=='Camp') {
