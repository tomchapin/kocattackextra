...
// Handy function to count the elements inside of an object
...
var KOCAttack={
	...
	seed:{},
	currentPage:null,
	cachedAttacks:{'initialized_time':0,'last_changed_time':0,'attacks':{}},
	
	CacheAttacks:function(){
		// Get all of the stored values in the system and filter out the relevant attacks
		...
		for(var n=0; n<names.length; n++) {
			...
			var value = this.browser_getValue(name);
			if(value=="") continue;
			var attack=JSON2.parse(value);
			if(!attack) continue;
			// Add the attack to the cache
			if(!this.cachedAttacks['attacks'][xVal]) this.cachedAttacks['attacks'][xVal] = new Object();
			if(!this.cachedAttacks['attacks'][xVal][yVal]) this.cachedAttacks['attacks'][xVal][yVal] = new Object();
			this.cachedAttacks['attacks'][xVal][yVal] = attack;
		}
		this.cachedAttacks['initialized_time'] = new Date().getTime();
		this.cachedAttacks['last_changed_time'] = this.cachedAttacks['initialized_time'];
		return this.cachedAttacks;
	},
	
	SaveCachedAttacksToDisk:function(){
		// Only save the cache to disk if it has changed since it was initialized
		if(this.cachedAttacks['last_changed_time'] > this.cachedAttacks['initialized_time']){
		}
	},
	...
	ShowOptionsDialog:function() {
		...
		div.innerHTML="<form><table>"+
			...
			"<input id='KOCAttackUnselectKnight' type='checkbox' "+(this.options.knightreset?'checked':'')+" /> Unselect knight when not in attack mode. <font color=#FF0000>(NEW)</font> <br />"+
	...
	*/	
	...
	SetAttackFromGuiXY:function(x,y,box,isSuicideWave,isTransport) {
		if(!isSuicideWave)
		...
		if(marchType==0 && !isTransport) {
			...
			//If there is no MM or supply troop sent then we decude it to be a normal wave
			var Suicide = false;
			if((troops[1]+troops[2]) != 0)
				if ((troops[1]+troops[2]+troops[10]+troops[12])==totalTroops)//Added cats to the mix
					Suicide = true;
			
			if(totalTroops>0 && (( Suicide && firstAttack) || isSuicideWave)) {
				if(totalTroops>0 && ((troops[1]+troops[2]+troops[10]+troops[12])==totalTroops && firstAttack)){
			...
			if(!attack.levelInfo){attack.levelInfo={'type':'','level':0};}

			//attack.ignore=undefined;
			this.prevAttack={'x':x,'y':y};
			this.SetAttack(x,y,attack);
			return attack;
		} else if(marchType==1 || isTransport) {
	...
	BulkAddCoords:function(box,coordsText,force,locationType,isSuicideWave) {
		if(!locationType){
			var locationType = ""; //Default to city
		}
		if(!isSuicideWave){
			var isSuicideWave = false;
		}
		...
		for(var r=0; r<coordRows.length; r++) {
			...
			var attack=this.SetAttackFromGuiXY(x,y,box,isSuicideWave, isTransport);
			if(attack) {
	...
	BulkAddAttackLink:function(box) {
		...
		var arrData = [["Camp", "Camps"],["", "City"],["Transport", "Transport"],["Wilderness", "Wildernesses"]];
		...
		bulkAdd.addEventListener('click',function() {
			...
			var locationType = ""; // Set default value to city
	...
	OnAttackBoxAppear:function(box) {
		...
		addBtn.addEventListener('click',function() {
			t.ClickMarch(null, box, true);
		},false);
	
		if(btnMarch)
			btnMarch.parentNode.insertBefore(addBtn,btnMarch);
	...
	//{"time":1273315720.514,"troops":"test","type":0,"messages":[["Attack (326,97)  - Barbarian Camp Lv. 2 (NewCity4216)","modal_messages_viewreports_view(\"529747\",1,51,2,0,\"Enemy\",\"0\",\"niknah\",\"M\",4,326,97,1273312617,1,333,110);return false;"],["Attack (326,97)  - Barbarian Camp Lv. 2 (NewCity4216)","modal_messages_viewreports_view(\"529747\",1,51,2,0,\"Enemy\",\"0\",\"niknah\",\"M\",4,326,97,1273312617,0,333,110);return false;"]]}
			...
			if(btnMarch)
				btnMarch.parentNode.insertBefore(deleteBtn,btnMarch);
	...
	ClickMarch:function(btnMarch, box, alert){
	var t = this;
	if(btnMarch){
	...
	var added = t.SetAttackFromGui(box);
	t.SetValuesCache();
	if(added)
		if(alert)
	...
	*/
	...
	NextAutoAttackCity:function() {
		...
		// Determine the first enabled city in the options
		var firstAvailableCityNum = 1;
	...
	*/
	...
	Listen:function() {
		...
		if(t.currentPage == "koc_game"){
			...
			// Get all of the attacks stored in greasemonkey and cache them in local memory
			this.CacheAttacks();
